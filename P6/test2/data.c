       ...
       /*     for(j=0;j<8;j++){
        ...
        for(i=0;i<SCALE;i++){
            ...
            switch(select){
                ...
                case 4: fprintf(fp,"%s $%d,%d($zero)\n",s_l_w[rand()%2],rand()%REG_NUM + REG_BEGIN,4*(rand()%25));break;
                case 5: fprintf(fp,"%s $%d,%d($zero)\n",s_l_h[rand()%3],rand()%REG_NUM + REG_BEGIN,2*(rand()%50));break;
                case 6: fprintf(fp,"%s $%d,%d($zero)\n",s_l_b[rand()%3],rand()%REG_NUM + REG_BEGIN,rand()%100);break;
                //case 7: fprintf(fp,"%s $%d,$%d\n",mu_di[rand()%2],rand()%REG_NUM + REG_BEGIN,rand()%REG_NUM + REG_BEGIN);break;
